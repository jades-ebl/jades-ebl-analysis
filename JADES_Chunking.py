""" JADES_Chunking.py
    ...
    Function to Mask sources for a chunk of the image data
...
def Final_Masking_Chunk(xmin,xmax,ymin,ymax,im_data,data_err,max_mag,filt,pix_scale,ID,x,y,s_maj,s_min,theta,bbox_xmin,bbox_xmax,bbox_ymin,bbox_ymax,npix):
    ...
    """  
    #print coordinates for chunking box
    print('xmin,xmax, ymin, ymax = ',xmin,' ,',xmax,' ,',ymin,' ,',ymax)

    #create list to append arrays of different stages of masking for histogram
    listhist=[]

    #Cut the array to correspond to chunking box
    chunk_data_all= im_data[ymin:ymax, xmin:xmax]
    chunk_data_err= data_err[ymin:ymax, xmin:xmax]
    #Place nan where there is no data or error
    chunk_data=np.where(chunk_data_err,chunk_data_all,np.nan)
    #Append array of image data before any masking to show in histogram
    listhist.append(chunk_data)

    #create mask where error becomes 0 and otherwise is 1
    #obj_mask=np.where(chunk_data_err,1,0)
    obj_mask=np.ones(chunk_data.shape)
    ...
    for i in range(len(ID)):
        if (xmin-100) <= x[i] and x[i]<= (xmax+100):
            if (ymin-100) <= y[i] and y[i]<= (ymax+100):
    ...
    for i in list_source:
        ...
        if thismag != np.inf and np.isnan(thismag)!=True:
            ...
            xmin_source = bbox_xmin[i]-xmin-50
            xmax_source = bbox_xmax[i]-xmin+50
            ymin_source = bbox_ymin[i]-ymin-50
            ymax_source = bbox_ymax[i]-ymin+50

            #cut the im array to the limits of each object
            im_source = chunk_data[ymin_source:ymax_source, xmin_source:xmax_source]
            ...
            object_center_pix = PixCoord(x[i]-xmin-xmin_source, y[i]-ymin-ymin_source)
    ...
    im_new=np.where(obj_mask==0,np.nan,chunk_data)
    ...
    #Run clip masking for 3 iterations
    while counter<3:
    ...
    print("The percent of original pixels left = ",(np.sum(np.isfinite(im_new))/np.sum(np.isfinite(chunk_data)))*100)
